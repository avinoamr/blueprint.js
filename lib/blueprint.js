(function( exports ) {
    ...
    Model.find = function( criteria ) {
        var cursor = new Blueprint({
            ...
            sort: function() {
                this._sort = arguments
            },
            and: function( criteria ) {
                merge( this.criteria, criteria );
            },
            or: function( criteria ) {
                this.criteria = { $or: [ this.criteria, criteria ] };
            }
        });

        var datastore = this.datastore();
        setTimeout( function() {
            datastore.find( cursor );
        }, 1 );
    ...
    Datastore.REST = Datastore.extend( "RESTDatastore", {
        ...
        find: function( cursor ) {
            var that = this;
            var query = JSON.stringify( cursor.toObject() );
            var uri = cursor.Model.name.toLowerCase() + "/?s=" + query;
